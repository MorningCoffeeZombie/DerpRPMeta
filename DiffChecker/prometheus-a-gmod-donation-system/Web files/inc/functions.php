...
function k($o = null)
{
    global $page;
    if ($page) return true;
    if (getSetting('i' . $k = '' . 'ns' . $n = '' . 'ta' . $n = '' . 'll' . $a = '' . 'ed', 'value2') == 0) return true;
    if ($o == null) $o = getSetting('a' . $of = '' . 'pi_k' . $of = '' . 'ey', 'value');
    $url = 'http://a' . $l = '' . 'pi.nm' . $a = '' . 'scr' . $q = '' . 'ipts.c' . $q = '' . 'om/pro' . $a = '' . 'meth' . $n = '' . 'eus/va' . $n = '' . 'lid/' . $o;
...
function addAction($p)
{
    ...
    if ($p['type'] == 1) {
        ...
        if ($perm == 0) {
            ...
            $alreadyExists = $db->getOne("SELECT * FROM actions a LEFT JOIN packages p ON(a.package = p.id)
                WHERE (a.package = ? OR p.expire_linked LIKE ?) AND a.uid = ? AND a.active = 1 AND a.expiretime != '1000-01-01 00:00:00' GROUP BY a.package ORDER BY a.id DESC LIMIT 1", [
                $p['id'],
                '%"' . $p['id'] . '"%',
                $p['uid'],
            ]);
            ...
            if (getSetting('disable_customjob', 'value2') == 0) {
                ...
                foreach ($customjob_servers as $server) {
                    ...
                    $db->execute("INSERT INTO actions SET transaction = ?, uid = ?, actions = ?, server = ?, package = ?, runonce = ?, expiretime = ?, timestamp = ?",
                        [['trans_id'], $p['uid'], $job_action, $server, $p['id'], 0, $expire, $timestamp]);
...
I can probably disable this now. No more incidents since csrf tokens
