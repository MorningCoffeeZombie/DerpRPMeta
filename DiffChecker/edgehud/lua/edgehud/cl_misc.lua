...
if EdgeHUD.Configuration.GetConfigValue( "CrashScreen" ) then
	...
	net.Receive("EdgeHUD:CrashScreenUpdate",function(  )
		...
		hook.Remove("HUDShouldDraw","EdgeHUD:RemoveCrosshair_CrashScreen")

		--Check if the crashScreen exists.
		if crashScreen:GetAlpha() > 0 then

			--Hide screen.
			crashScreen:AlphaTo(0,0.3,0, function(  )
				crashScreen:SetVisible(false)
			end)
	...
	hook.Add("Think","EdgeHUD:CheckServerConnection", function(  )
		...
		if nextCheck <= CurTime() then
			...
			if RealTime() - lastPing > 3 and EdgeHUD.Disconnected == false then
...
if VC and EdgeHUD.Configuration.GetConfigValue( "Vehicle_DamageIndicators" )  then
	...
	for k,v in pairs(indicators) do
		...
		indicators[k].Element:SetPos(startPos + (VARS.ElementsMargin + VARS.WidgetHeight) * k,screenHeight - VARS.ElementsMargin - VARS.WidgetHeight)
	...
	timer.Create("EdgeHUD:UpdateVehicleIndicators",0.2,0,function(  )
		...
		if didChange == true then
			...
			for k,v in pairs(indicators) do
				...
				--Set the position.
				v.Element:SetPos(nextPos,screenHeight - VARS.ElementsMargin - VARS.WidgetHeight)
...
if LevelSystemConfiguration and (DarkRPLevelSystem == "Show on change" or DarkRPLevelSystem == "Always show") then
	...
	levelWidget:SetPos(screenWidth / 2 - barWidth / 2,screenHeight * 0.03)
...
if ItemPickups == "EdgeHUD Design" then
	...
	local itemPickupWidth = math.floor(screenHeight * 0.18)
	...
	local itemMargin = math.floor(itemPickupHeight * 0.02)
	...
	itemPickupFrame.Paint = function()
		...
		for k,v in pairs(items) do
			...
			surface.DrawOutlinedRect(math.Round(v.lerpedXPos),math.Round(v.lerpedYPos),itemPickupWidth,itemHeight)

			--Draw the corners.
			surface.SetDrawColor(ColorAlpha(COLORS["White_Corners"], v.alpha * COLORS["White_Corners"].a))
			EdgeHUD.DrawEdges(math.Round(v.lerpedXPos),math.Round(v.lerpedYPos),itemPickupWidth,itemHeight, 8)

			draw.SimpleText( v.text, "EdgeHUD:ItemPickup", math.Round(v.lerpedXPos) + itemPickupWidth / 2, math.Round(v.lerpedYPos) + itemHeight / 2, ColorAlpha(COLORS["White"], v.alpha * COLORS["White"].a), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
if EdgeHUD.LatestVersion != EdgeHUD.Version and EdgeHUD.LatestVersion != "" and (EdgeHUD.Owner == ply:SteamID64() or table.HasValue(groupsAccess, ply:GetUserGroup())) then
	...
	informationText:SetSize(OutdatedFrame:GetWide() * 0.7,OutdatedFrame:GetTall() * 0.6)
