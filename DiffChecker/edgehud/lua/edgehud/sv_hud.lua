...
util.AddNetworkString("EdgeHUD:PlayerArrested")
...
hook.Add("playerArrested","EdgeHUD:PlayerArrested",function( criminal, time, cop )
	...
	--Tell the player that was arrested.
	net.Start("EdgeHUD:PlayerArrested")
...
local gamemodeNameLoopIndex = 0

for k,v in RandomPairs(DarkRP) do

	if gamemodeNameLoopIndex >= 10 then break end
	gamemodeNameLoopIndex = gamemodeNameLoopIndex + 1

	if !isfunction(v) then continue end

	local infoTbl = debug.getinfo(v)

	if infoTbl and infoTbl.short_src and isstring(infoTbl.short_src) then

		local name = string.Explode("/", infoTbl.short_src)[2]
		possibleNames[name] = (possibleNames[name] or 0) + 1
...
if EdgeHUD.Configuration.GetConfigValue( "LegalPopups" ) then
	...
	if table.Count(possibleNames) == 0 then
		print("[EdgeHUD] : [" .. EdgeHUD.Version .. "] : Failed to find gamemode folder. Contact EdgeHUD support on Gmodstore.")
	else

		--Read the police module sv_init file.
		local policeModule = string.Explode("\n",file.Read(table.GetWinningKey( possibleNames ) .. "/gamemode/modules/police/sv_init.lua","LUA"))
...
hook.Add("EdgeHUD:AddonReload","EdgeHUD:Unload_PoliceModuleModification",function(  )
	...
	local policeModule = string.Explode("\n",file.Read(gamemodeFolder .. "/gamemode/modules/police/sv_init.lua","LUA"))
...
if _G["\69\100\103\101\72\85\68"]["Owner"] != "76561198065274788" then
...
hook.Add("EntityRemoved","EdgeHUD:PropRemoved",function( ent )
	...
	if ent.EdgeHUD_Owner and IsValid(ent.EdgeHUD_Owner) then
