	...
	if ply:InVehicle() then
		...
		if IsValid(curWep) then
			...
			if primAmmo != -1 then
				...
				sparePrimary = ply:GetAmmoCount(primAmmo)
				widgetsDrawing.mgangs = false
			end
			...
			if secAmmo != -1 then
				...
				spareSecondary = ply:GetAmmoCount(secAmmo)
				widgetsDrawing.mgangs = false
			end
...
local extraWidgets = {
	...
	},
	{
		...
		getData = function() return secondary == -1 and spareSecondary or secondary .. "/" .. spareSecondary end,
		shouldDraw = function() return widgetsDrawing.secAmmo end
	},
	{
		icon = Material("edgehud/icon_primaryammo.png","smooth"),
		getData = function() return primary == -1 and sparePrimary or primary .. "/" .. sparePrimary end,
...
timer.Create("EdgeHUD:ExtraWidgets",0.05,0,function(  )
	...
	if ply:InVehicle() and Photon and ply:GetVehicle():IsEMV() then
		...
		xPos = screenWidth - VARS.ScreenMargin - VARS.extraWidgetsWidth
	end

	--Cache the next position.
	local nextPos = screenHeight - VARS.ScreenMargin - VARS.WidgetHeight
