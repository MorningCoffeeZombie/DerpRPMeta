		...
		for k,v in pairs(playersToRender) do
			...
			cam.Start3D2D(Vector(eyePos.x, eyePos.y,v:GetPos().z) + (inVehicle and Vector(0,0,60) or Vector(0,0,math.max(playerHeightOffset + 18, 55))),ply:InVehicle() and Angle(0,ply:GetVehicle():GetAngles().y + eyeAngs.y - 90,90) or Angle(0,eyeAngs.y - 90,90),0.1)
				...
				elseif !inVehicle then
					...
					--Draw icon.
					surface.SetDrawColor(COLORS["White"])
					surface.SetMaterial(v:IsSpeaking() and MAT_SPEAKER or v:IsTyping() and MAT_CHAT or v:getDarkRPVar("wanted") == true and MAT_WANTED or hasLicense and MAT_LICENSE or MAT_USER)	
					...
					draw.SimpleText(jobname,"EdgeHUD:3D2D:Small",boxPos + boxSize + margin,boxSize,team.GetColor(v:Team()), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
