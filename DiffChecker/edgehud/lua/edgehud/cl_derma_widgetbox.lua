...
hook.Add("Think","EdgeHUD:ShouldDrawHUD",function(  )
	...
	if nextTimerCheck > CurTime() then return end
	nextTimerCheck = CurTime() + 0.5

	--Check if cl_drawhud is set to 1/0.
	if GetConVar("cl_drawhud"):GetInt() == 0 or gui.IsGameUIVisible() or EdgeHUD.Disconnected == true then
